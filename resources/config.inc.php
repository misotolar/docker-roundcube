...
$config['imap_host'] = sprintf('%s:%d', getenv('ROUNDCUBEMAIL_DEFAULT_HOST'), getenv('ROUNDCUBEMAIL_DEFAULT_PORT'));
$config['smtp_host'] = sprintf('%s:%d', getenv('ROUNDCUBEMAIL_SMTP_SERVER'), getenv('ROUNDCUBEMAIL_SMTP_PORT'));
...
foreach (glob('/usr/local/etc/roundcube/*.inc.php') as $filename) {
    include($filename);
}
